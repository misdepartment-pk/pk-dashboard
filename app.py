...
total_bills = df_filtered["ORDER_COUNT"].sum() if not df_filtered.empty else 0.0
avg_bill = total_sales / total_bills if total_bills > 0 else 0.0
...
with tab_table:
  ...
  if not df_filtered.empty:
    if "ORDER_COUNT" not in df_filtered.columns:
      df_filtered["ORDER_COUNT"] = 1
    ...
    # ป้องกันเรื่องชื่อคอลัมน์ติดช่องว่างส่วนเกิน
    summary_table.columns = [str(col).strip() for col in summary_table.columns]

    # คำนวณค่าเฉลี่ยต่อบิลอย่างปลอดภัย (ป้องกัน KeyError และ Division by Zero)
    if "ยอดขายรวม" in summary_table.columns and "จำนวนบิล" in summary_table.columns:
      summary_table["เฉลี่ยต่อบิล"] = np.where(
          summary_table["จำนวนบิล"] > 0,
          summary_table["ยอดขายรวม"] / summary_table["จำนวนบิล"],
          0.0,
      )
    else:
      summary_table["เฉลี่ยต่อบิล"] = 0.0
